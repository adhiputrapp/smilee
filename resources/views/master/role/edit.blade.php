<x-app-layout>
    <h1 class="text-3xl font-bold border-b-2">Edit Permission Role</h1>
    <div class="py-12">
            ...
            <div id="datatable" class="bg-gradient-to-tr from-slate-300 to-slate-400 overflow-hidden shadow-sm sm:rounded-lg p-4 mb-4">
                ...
                <form action="{{ route('roles.update', [ 'id' => $role->id]) }}" method="POST">
                    ...
                    <div class="form-group mb-5">
                        ...
                        <x-text-input type="text" name="name" id="name" value="{{$role->name}}" class="block mt-1 w-full" required/>
                                                ...
                                                @foreach ($permissions as $item)
                                                    <tr class="border-b dark:border-neutral-500">
                                                        <td class="whitespace-nowrap px-6 py-4 font-medium">
                                                            <input type="checkbox" name="permissions[{{ $item->name }}]" value="{{ $item->name }}" class='permission' {{ in_array($item->name, $rolePermissions) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        $('[name="permissions[all_permission]"]').on('click', function() {
            if ($(this).is(':checked')) {
