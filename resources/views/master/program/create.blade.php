<x-app-layout>
    <h1 class="text-3xl font-bold border-b-2">Input Program</h1>
    <div class="py-12">
            ...
            <div class="bg-gradient-to-tr from-slate-300 to-slate-400 overflow-hidden shadow-sm sm:rounded-lg p-4 mb-4">
                ...
                <form action="{{ route('programs.store') }}" method="POST">
                    ...
                    <div class="form-group mb-5">
                        ...
                        <x-text-input type="text" name="kode_program" id="kode_program" class="block mt-1 w-full" required/>
                    </div>
                    <div class="form-group mb-5">
                        <x-input-label for="nama_program" :value="__('Nama Program')"/>
                        <x-text-input type="text" name="nama_program" id="nama_program" class="block mt-1 w-full" required/>
                    </div>
                    <div class="form-group mb-5">
                        <x-input-label for="nama_biro_relasi" :value="__('Nama biro')"/>
                        <select id="nama_biro_relasi" name="nama_biro_relasi" class="block mt-1 w-full" required>
                            <option value="">-- Pilih nama biro --</option>
                            @foreach ($biros as $biro)
                                <option value="{{$biro->nama_biro}}">{{$biro->nama_biro}}</option>
                            @endforeach
                        </select>
                    </div>
