    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gradient-to-tr from-slate-300 to-slate-400 overflow-hidden shadow-sm sm:rounded-lg p-4 mb-4">
                <form action="{{ route('pelimpahans.update', ['id' => $pelimpahans->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-5">
                        <x-input-label for="nodokumen" :value="__('No Dokumen')"/>
                        <x-text-input type="text" name="nodokumen" id="nodokumen" class="block mt-1 w-full" required value="{{ $pelimpahans->nodokumen}}"/>
                    </div>
                    <div class="form-group mb-5">
